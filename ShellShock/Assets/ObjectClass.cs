...
//					particleSpawn.Spawn ();
					...
					if (type == Type.RIGHTBOOMERANG) {
						sp.transform.rotation = Quaternion.Euler (new Vector3 (2 * (Spawner.spawner.joyStick.angle) - (this.transform.rotation.x + 180), this.transform.rotation.y, this.transform.rotation.z));
					} else if (type == Type.LEFTBOOMERANG) {
						sp.transform.rotation = Quaternion.Euler (new Vector3 (3 * (this.transform.rotation.x + 180) - 2 * (Spawner.spawner.joyStick.angle), this.transform.rotation.y, this.transform.rotation.z));
					}
